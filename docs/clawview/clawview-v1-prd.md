...
5. 采用“够用优先”原则：优先保证趋势判断与异常发现，不追求 v1 的 100% 指标精确度。

## 3. 非目标（Non-Goals）
1. 不提供暂停任务/重启服务等控制功能。
2. 不做复杂链路追踪与分布式调试。
3. v1 不做多租户权限系统。
4. v1 不为“绝对精确”增加重度采集链路（以业务可用性优先）。
...
### 5.1 监控采集模式（v1 决策）
1. 采用 **Hook 触发式增量读取**：触发一次，读取一次；不跑常驻实时监控。
2. 采用游标增量（cursor）读取，避免重复统计与漏读。
3. v1 暂不引入 cron 兜底补刷；如后续实测出现漏数，再最小化补充。

### 5.2 外发与隐私基线（必须满足）
1. 数据需要外发至用户后端数据库，但仅允许白名单字段（计数/状态码/分组/耗时）。
2. 禁止外发：消息正文、请求/响应 body、token、cookie、授权头等敏感内容。
3. 先本地脱敏再出网；传输必须使用 TLS，并建议请求签名（HMAC）防篡改。
4. 后端按 tenant/project 强隔离，且支持最小保留策略与审计追踪。
...
9. 消息阻塞次数（24h）
10. 响应超时次数（24h）
> [更新 2026-02-28] 「慢处理」命名弃用，改为「消息阻塞 / 响应超时」，仅统计系统事件处理阻塞，不包含业务任务本身执行时长。
...
### 6.1 可采集性说明（与上线门槛直接相关）
1. PRD 字段仅定义“需要展示什么”，是否可上线由 Plan 的 P0-Core 可采集性门槛判定。
2. 首页字段必须是 Ready/Derived，不允许 Gap 字段直接上首页。
3. Gap 字段必须显示 `--` +「数据未接入」，并进入缺口清单跟踪。
4. 首发门槛：P0-Core 覆盖率 >= 98%，口径一致性抽检 100%。
...
|---|---|---|---|
| 健康总览条 | 运行状态、异常重启(24h)、限速比例(24h)、系统错误数、消息阻塞(24h)、响应超时(24h) | 固定 1 行 | 明细日志、完整错误列表 |
...
11. 采集模式符合 Hook 触发式增量读取（含 cursor 去重），且不依赖常驻实时监控
12. 外发链路满足白名单与脱敏基线（不外发正文/token 等敏感字段）
